...
with st.form("search_parameters_form"):
    with col1:
        # Get format and pull relevant prompt
        format_choice = st.radio("Choose the preferred output format:", ['Summary', 'Informational Event'], index=None)
        if format_choice == 'Informational Event':
    ...
    with col2:
        ...
        if search_option == 'Specific Indexes' and selected_indexes:
            selected_index = ",".join(selected_indexes)
        elif search_option == 'All Project Indexes' and project_choice:
            selected_indexes = project_indexes[project_choice]
            selected_index = ",".join(selected_indexes)
        else:
            selected_index = None
        ...
        if selected_index:
            category_values_one, category_values_two, language_values, country_values = populate_default_values(
                selected_index, es_config)

            categories_one_selected = st.multiselect(
                'Select categories of the first (or only) level', category_values_one,
                default=['Any'])

            if "dem-arm" in selected_index:
                categories_two_selected = st.multiselect(
                    'Select categories of the second level', category_values_two,
                    default=['Any'])

            languages_selected = st.multiselect('Select languages', language_values,
                                                default=['Any'])

            countries_selected = st.multiselect('Select countries', country_values,
                                                default=['Any'])
